        ...
        for r in results:
            for box in r.boxes:
                ...
                if class_name in FOOD_DATA:
                    ...
                    carbs = FOOD_DATA[class_name]['carbs']
                    glycemic_load = (gi * carbs) / 100  # Calculate glycemic load
                    response['glycemic_load'][class_name] = glycemic_load
                    response['total_glycemic_load'] += glycemic_load
                else:
                    response['glycemic_load'][class_name] = 0.0  # Default value for unknown foods

        # Add glycemic load category based on total glycemic load
        if response['total_glycemic_load'] < 10:
            response['glycemic_load_category'] = 'Low glycemic load'
        elif response['total_glycemic_load'] < 20:
