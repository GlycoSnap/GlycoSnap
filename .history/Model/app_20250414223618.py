...
try:
    if os.path.exists(model_path):
        model = YOLO(model_path)  # Auto-detect task
...
def predict_glycemic_load():
    try:
        ...
        # Run YOLO inference
        try:
            # Use imgsz to avoid size issues, no explicit task
            results = model.predict(tmp.name, save=False, conf=0.5, device='cpu', imgsz=640)
