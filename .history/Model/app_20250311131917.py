...
def predict_glycemic_load():
    try:
        ...
        results = yolo_model.predict(image, imgsz=320, conf=0.5, task="segment")
        ...
        for r in results:
            for mask, box in zip(r.masks.xy, r.boxes):
                ...
                if class_name in FOOD_DATA:
                    ...
                    # Convert to cm² (Ensure scaling factor is correct)
                    pixel_to_cm2 = 0.001  # Adjust this based on testing
        ...
        for food_item, area_cm2 in segmented_areas.items():
            ...
            gi = food_info["gi"]
            carb_density = food_info["carb_density"]
            ...
            gl = min(gl, 100)  # Prevent unrealistic GL values
