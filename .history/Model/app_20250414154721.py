...
try:
    ...
    app.logger.info(f"Model classes: {model.names}")
...
def predict_glycemic_load():
    try:
        ...
        # Run YOLO inference
        try:
            results = model.predict(tmp.name, save=False, conf=0.5, device='cpu')
            app.logger.info(f"YOLO results length: {len(results)}")
        except Exception as e:
            app.logger.error(f"YOLO inference failed: {str(e)}")
            raise
        finally:
            tmp.close()
