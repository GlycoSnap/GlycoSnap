...
def predict_glycemic_load():
    try:
            ...
            try:
                ...
                if hasattr(result, 'masks') and result.masks is not None and hasattr(result, 'names'):
                    for mask, cls, conf in zip(result.masks.xy, result.boxes.cls, result.boxes.conf):
                        try:
                            class_id = int(cls)
                            app.logger.info(f"Detected class: {class_id} ({result.names.get(class_id, 'Unknown')}) with confidence {conf}")
        ...
        response['detected_foods'] = list(response.get('detected_foods', []))
        response['known_foods'] = []
        response['unknown_foods'] = []
        
        for food, area in segmented_areas.items():
            if food in FOOD_DATA:
                response['known_foods'].append(food)
            else:
                response['unknown_foods'].append(food) 
