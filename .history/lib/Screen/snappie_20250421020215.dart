...
class _ChatPageState extends State with TickerProviderStateMixin { 
  ...
  final List<Map<String, String>> _messages = []; final ScrollController _scrollController = ScrollController();
...
Future _pickImage() async { 
  ...
  if (image != null) { setState(() { _messages.add({ 'type': 'query', 'text': '', 'image': image.path, }); _isLoading = true; });
...
@override Widget build(BuildContext context) { return Align( alignment: Alignment.centerLeft, child: Container( padding: EdgeInsets.all(8.0), decoration: BoxDecoration( color: white.withOpacity(0.1), borderRadius: BorderRadius.circular(15), ), child: Row( mainAxisSize: MainAxisSize.min, children: List.generate(3, (index) => AnimatedBuilder( animation: _controller, builder: (context, child) { return Container( width: 8, height: 8, margin: EdgeInsets.symmetric(horizontal: 2), decoration: BoxDecoration( shape: BoxShape.circle, color: white.withOpacity(0.5 + (0.5 * ((_animation.value + index / 3) % 1))), ), ); }, )), ), ), ); } }