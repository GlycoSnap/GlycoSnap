...
class _Question3State extends State<Question3> {
  ...
  final Map<String, dynamic> profileData;
  const CreateAccount({super.key, required this.profileData}) : super(key: key);: super(key: key);
...
class _CreateAccountState extends State<CreateAccount> {
  ...
  Future<void> _createAccountWithEmailAndPassword() async {
    ...
    try {
      ...
      if (res.error != null) {
        // Supabase-side error (e.g. weak password, already registered)
        Get.snackbar('Error', res.error!.message, snackPosition: SnackPosition.BOTTOM);
        return;
      }

      final User? sbUser = res.user;
      if (sbUser == null) {
        Get.snackbar('Error', 'Sign‑up failed. Please try again.', snackPosition: SnackPosition.BOTTOM);
        return;
      }

      // 2) Persist your extra profile fields into the `users` table
