...
// Helper function to scroll to the bottom void scrollToBottom() { WidgetsBinding.instance.addPostFrameCallback(() { _scrollController.animateTo( _scrollController.position.maxScrollExtent, duration: const Duration(milliseconds: 300), curve: Curves.easeOut, ); }); }

// Function to pick and analyze an image Future _pickImage() async { final XFile? image = await _picker.pickImage(source: ImageSource.gallery); if (image != null) { setState(() { _messages.add({ 'type': 'query', 'text': '', 'image': image.path, }); _isLoading = true; }); _scrollToBottom();