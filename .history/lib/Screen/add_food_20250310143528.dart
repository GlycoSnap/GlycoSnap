...
class _AddFoodState extends State<AddFood> {
  ...
  final String baseUrl =
      'http://192.168.100.12:5000/predict'; // Replace with your Flask server URL
  ...
  if (_imageFile == null) {
    ...
    return {'glycemic_load': 0.0, 'total_glycemic_load': 0.0}; // Return default values
  }
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      return {
        'glycemic_load': data['glycemic_load'] ?? 0.0, // Ensure default value
        'total_glycemic_load': data['total_glycemic_load'] ?? 0.0, // Ensure default value
        'glycemic_load_category': data['glycemic_load_category'] ?? 'Unknown', // Ensure default value
        'food_name': data['food_name'] ?? 'Unknown Food', // Ensure default value
      };
    } else {
  ...
  builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      ...
      final glycemicLoadCategory = data['glycemic_load_category'] ?? 'Unknown';
      Color categoryColor;
      ...
      return Column(
        ...
        children: [
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ...(data['glycemic_load'] as Map<String, dynamic>?)?.entries.map((entry) {
                    return Padding(
                      padding: const EdgeInsets.symmetric(vertical: 4.0),
                      child: Text(
                        '${entry.key}: ${(entry.value as num?)?.toStringAsFixed(2) ?? 'N/A'}',
                  ...
                  RichText(
                    text: TextSpan(
                      children: [
                        ...
                        TextSpan(
                          text: (data['total_glycemic_load'] as num?)?.toStringAsFixed(2) ?? 'N/A',
                          style: TextStyle(
                            fontFamily: 'PoppinsBold',
                            fontSize: 16,
                            color: categoryColor,
          ...
          ElevatedButton(
            onPressed: () {
              final foodName = data['food_name']?.toString() ?? 'Unknown Food';
              final totalGL = (data['total_glycemic_load'] as num?)?.toDouble() ?? 0.0;

              _showMealSelectionDialog(context, foodName, totalGL);
            },
