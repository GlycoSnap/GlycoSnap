...
def predict_glycemic_load():
    try:
        ...
        segmented_areas = {}
        # Handle rf_result as list or dict
        ...
        for pred in predictions:
            try:
                if not isinstance(pred, dict):
                    ...
                    continue
                # Try possible class keys
                class_name = pred.get("class", pred.get("label", pred.get("class_name")))
                if not class_name:
                    app.logger.warning(f"No class/label in prediction: {pred}")
                    continue
                if "mask" not in pred:
                    app.logger.warning(f"Missing mask in prediction: {pred}")
                    continue
                class_name = class_name.lower()
                mask_pts = pred["mask"]
                # Validate mask_pts
                if not isinstance(mask_pts, list):
                    app.logger.warning(f"Invalid mask format: {mask_pts}")
                    continue
                area_pixels = cv2.contourArea(np.array(mask_pts, dtype=np.float32))
