...
class _AddFoodState extends State<AddFood> {
  ...
  builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      ...
      return const Text("Error calculating glycemic load");
    }

    // Ensure valid value before formatting
    double glycemicLoad = (snapshot.data?['glycemic_load'] as num?)?.toDouble() ?? 0.0;

    return Text("Glycemic Load: ${glycemicLoad.toStringAsFixed(2)}");
  },
),
  ...
  if (_imageFile == null) {
    ...
    return {'glycemic_load': 0.0}; // Return default value
  }
  ...
  try {
    ...
    return {'glycemic_load': 0.0}; // Return default value on failure
  }
}
                          ...
                          if (snapshot.connectionState ==
                            ...
                            return Column(
                              ...
                              children: [
                                ...
                                const SizedBox(
                                    height: 20), // Adds spacing before button
                                ElevatedButton(
                                  onPressed: () {
                                    final foodName =
