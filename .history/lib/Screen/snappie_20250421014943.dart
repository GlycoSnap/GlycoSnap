...
Future _pickImage() async { 
  ...
  if (image != null) { 
    setState(() { 
      ...
      _messages.add({ 'type': 'query', 'text': '', 'image': image.path, }); 
      // Clear the previously selected image 
      image = null; }); 
      // Scroll to the latest message 
      WidgetsBinding.instance.addPostFrameCallback(() { _scrollController.jumpTo(_scrollController.position.maxScrollExtent); }); 
      } 
      }

// Function to send image to backend 
Future _sendImage() async { 
  if (_image == null && _messages.isEmpty) 
  { setState(() { _messages.add({ 'type': 'response', 'text': 'Please select an image first.', }); }); return; }
