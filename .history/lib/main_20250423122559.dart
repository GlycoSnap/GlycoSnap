...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetMaterialApp(
      ...
      // Use AuthWrapper as the very first screen:
      home: const AuthWrapper(),

      // Traditional named routes for slides / login / etc:
      getPages: [
...
/// Checks Supabase auth + ensures a row exists in `users`.
/// If valid → MainScreen, otherwise → splash screen.
class AuthWrapper extends StatelessWidget {
  const AuthWrapper({Key? key}) : super(key: key);

  Future<bool> _hasValidProfile() async {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<bool>(
      ...
      builder: (context, snap) {
        ...
        // either no session or no user-row → show splash
...
class _MainScreenState extends State<MainScreen> {
  ...
  final List<Widget> _pages = const [
    HomePage(), Review(), AddFood(), Community(), Settings()
  ];
